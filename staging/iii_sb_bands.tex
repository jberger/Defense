\begin{tikzpicture}
  ...
  \draw<4-> (gamma 7) arc [start angle=90,  end angle=30,   radius=1.5]
    node [below right] {$\Gamma_{7}$};
  \draw<4-> (gamma 7) arc [start angle=-90, end angle=-150, radius=1.5];
  \draw<4-> [green,->] (gamma 7) ++(0,0.15) arc [start angle=90,  end angle=30,   radius=1.5]
    node [right] {$\tau_{{\scriptscriptstyle decay}}$};

  % defined energies
  \coordinate (energies) at (-1.5,0);
  \draw<5-> [thick] ($(gamma 8) + (-2,0)$) -- ++(4,0);
  \draw<5-> [<->] (energies) -- (energies |- gamma 8)
    node [pos=0.5,left] {$E_{g}^{\prime}$};

  \coordinate (vacuum level) at ($(gamma 8) + (0,0.6)$);
  \draw [thick,red] ($(vacuum level) + (-2,0)$) -- ++(4,0);
  \draw<5-> [<->] (energies |- gamma 8) -- (energies |- vacuum level)
